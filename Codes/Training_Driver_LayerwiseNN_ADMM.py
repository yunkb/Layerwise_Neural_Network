...
class RunOptions:
    def __init__(self, hyper_p): 
        ...
        self.filename = data_type + '_ADMM_mhl%d_r%s_p%s_nTOL%s_eTOL%s_b%d_e%d' %(hyper_p.max_hidden_layers, regularization_string, penalty_string, node_TOL_string, error_TOL_string, hyper_p.batch_size, hyper_p.num_epochs)
...
def trainer(hyper_p, run_options):
    ...
    while loss_value > hyper_p.error_TOL and hidden_layer_counter < hyper_p.max_hidden_layers:       
        ...
        with tf.Session(config=gpu_config) as sess:
            ...
            #=== Save final model ===#
            save_weights_and_biases(sess, hyper_p, hidden_layer_counter, run_options.NN_savefile_name, 1)
