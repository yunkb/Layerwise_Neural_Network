...
def save_weights_and_biases(sess, hyper_p, weight_list_counter, savefilepath, thresholding_flag):
    ...
    trained_weights = sess.run("NN_layerwise/W" + str(l+1) + ':0')
    trained_biases = sess.run("NN_layerwise/b" + str(l+1) + ':0')
    ...
    trained_output_weights = sess.run("NN_layerwise/W" + str(l+2) + ':0')
    trained_output_biases = sess.run("NN_layerwise/b" + str(l+2) + ':0')
    if thresholding_flag == 1:
